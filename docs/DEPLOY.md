...
|------------|--------------|----------|
| `PORT` | Нет (по умолчанию 3000) | Порт, на котором слушает приложение. При использовании nginx обычно оставляют 3000. |
...
Минимальный production без истории и Telegram: переменные не задавать — приложение будет работать на порту 3000.
...
Приложение будет доступно на порту **3000**. Скриншоты сохраняются в Docker volume `screenshots` и не пропадут при перезапуске.
